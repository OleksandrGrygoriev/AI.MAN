(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) {
            ...
            new core(".remarks__slider", {
                ...
                loop: true,
                navigation: {
                    prevEl: ".remarks__button_prev",
                    nextEl: ".remarks__button_next"
                }
            });
        }
    }
    ...
    function moveImage() {
        const image01 = document.querySelector(".intro__01-image");
        if (image01) setTimeout((() => {
            image01.classList.add("visible");
        }), 2e3);
    }
    const tabsNav = document.querySelector(".tabs-technologies__nav");
    tabsNav.onclick = event => {
        const targetElement = event.target;
        if (targetElement.closest(".tabs-technologies__button")) {
            const tabNavButton = targetElement.closest(".tabs-technologies__button");
            if (!tabNavButton.classList.contains("_active")) {
                const activeTabNavButton = document.querySelector(".tabs-technologies__button._active");
                activeTabNavButton.classList.remove("_active");
                tabNavButton.classList.add("_active");
                const tabItems = document.querySelectorAll(".tab-technologies");
                const activeTabItem = document.querySelector(".tab-technologies._active");
                activeTabItem.classList.remove("_active");
                tabItems[getIndex(tabNavButton)].classList.add("_active");
            }
        }
